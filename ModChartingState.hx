...
class ModChartingState extends MusicBeatState
{
	...
	[
		...
		['move note in', "value 1: note num in notes array [0], [1], [2], [3] \nvalue 2: duration of tween\nvalue 3: added x position\nvalue 4: added y position\nvalue 5: Opacity\nvalue 6: added angle"],
		['move note out', "value 1: note num in notes array [0], [1], [2], [3] \nvalue 2: duration of tween\nvalue 3: added x position\nvalue 4: added y position\nvalue 5: Opacity\nvalue 6: added angle"],
		['move note linear', "value 1: note num in notes array [0], [1], [2], [3] \nvalue 2: duration of tween\nvalue 3: added x position\nvalue 4: added y position\nvalue 5: Opacity\nvalue 6: added angle"],
		['move note curve', "value 1: note num in notes array [0], [1], [2], [3] \nvalue 2: duration of tween\nvalue 3: added x position\nvalue 4: added y position\nvalue 5: Opacity\nvalue 6: added angle\nNote will curve back to original location."],
		['move note lock', "value 1: note num in notes array [0], [1], [2], [3] \nvalue 2: duration of tween\nvalue 3: added x position\nvalue 4: added y position\nvalue 5: Opacity\nvalue 6: added angle\nnotes are locked to rotation of arrow"]
	];
	...
	**/
	...
	override function create()
	{
		...
		add(zoomTxt);
		getfake = new FlxTypedGroup<StrumNote>();
		for (i in 0...8){
			trace("penis");
			var note:StrumNote = new StrumNote(850 + (i * 56), FlxG.height - 600, i, 0, i);
			if(i >= 4) {
				note.x += 160;
			}
			note.ogposx = note.x * 2;
			note.ogposy = note.y * 2;
			note.setGraphicSize(56, 56);
			...
			for (event in _song.events)
			{
				...
				if(endThing > event[0] && event[0] >= startThing)
				{
					...
					for (i in 0...event[1].length)
					{
						...
						copiedEventArray.push([eventToPush[0], eventToPush[1], eventToPush[2], eventToPush[3], eventToPush[4], eventToPush[5], eventToPush[6]]);
					}
	...
	function addEventsUI():Void
	{
		...
		descText = new FlxText(20, 300, 0, eventStuff[0][0]);
		...
		for (i in _song.events)
		{
			...
			if (gay == true) {
				...
				if (note.howtocode < 1 && note.howtocode > 0 && paused == true) {
					...
					getfake.members[note.valyer1].epicpenis = FlxTween.tween(getfake.members[note.valyer1], {"y": (getfake.members[valyer1].ogposy + note.valyer4), "x": (getfake.members[note.valyer1].ogposx + note.valyer3), "angle": (note.valyer6), "alpha": (note.valyer5)}, 1, {ease: FlxEase.quadInOut});
	...
	function updateGrid():Void
	{
		...
		for (i in _song.events)
		{
			if(endThing > i[0] && i[0] >= startThing)
			{
				...
				var text:String = note.eventName;
				if(note.eventLength > 1) text = note.eventLength + ' Events:\n' + note.eventName;

				var daText:AttachedFlxText = new AttachedFlxText(note.width / 2, -note.height / 2, 400, text, 12);
