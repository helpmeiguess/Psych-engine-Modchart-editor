...
import editors.ModChartingState;
...
class PlayState extends MusicBeatState
{
	...
	#end
	var bluevoi:FlxSprite;
	var bluevoid:FlxEffectSprite;
	var glish:FlxWaveEffect;
	...
	public var doKill:Bool = false;
	public var doKill2:Bool = false;
	public var doKill3:Bool = false;
	public var doKill4:Bool = false;
	public var b:Bool = false;
	public var b2:Bool = false;

	public var songSpeedTween:FlxTween;
	public var effectTween:FlxTween;
	...
	public var urmom:FlxPoint; //because this value is used as a parent, funny humor lololol
	public var urchild:FlxPoint; //because this value is used as the child, funny humor lololol
	public var killplace:FlxPoint;
	public var killplace2:FlxPoint;
	public var weanor:FlxTrail;
	...
	public var pissballs:FlxTypedGroup<StrumNote>;
	...
	public var ween:Bool = ClientPrefs.downScroll;
	...
	public static var valya1:Int = 0;
	public static var valya1_2:Int = 0;
	public static var valya1_3:Int = 0;
	public static var valya1_4:Int = 0;
	public static var valya2:Float = 0;
	public static var valya3:Int = 0;
	public static var valya4:Int = 0;
	public static var valya5:Float = 0;
	public static var valya6:Int = 0;
	public static var pissandshit:Int = 500;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'blue void': //(not) Week 2
				bluevoi = new BGSprite('Bluevoid', -1400, 100, 0.4, 0.4);
				bluevoi.angle += 5;
				bluevoid = new FlxEffectSprite(bluevoi);
				bluevoid.x -= 600;
				bluevoid.y -= 400;
				glish = new FlxWaveEffect(FlxWaveMode.ALL, 10, 0, 3, 5);
				bluevoid.effects = [glish];
				add(bluevoid);
			case 'philly': //Week 3
				if(!ClientPrefs.lowQuality) {
	...
	private function generateSong(dataPath:String):Void
	{
		...
		if (OpenFlAssets.exists(file)) {
			...
			for (event in eventsData) //Event Notes
			{
				...
				{
					var newEventNote:Array<Dynamic> = [event[0], event[1][i][0], event[1][i][1], event[1][i][2], event[1][i][3], event[1][i][4], event[1][i][5], event[1][i][6]];
					var subEvent:Array<Dynamic> = [newEventNote[0] + ClientPrefs.noteOffset - eventNoteEarlyTrigger(newEventNote), newEventNote[1], newEventNote[2], newEventNote[3], newEventNote[4], newEventNote[5], newEventNote[6], newEventNote[7]];
	...
	override public function update(elapsed:Float)
	{
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				/* scroll directions cool
				var urmom = new FlxPoint(playerStrums.members[daNote.noteData].x, playerStrums.members[daNote.noteData].y);
				var urchild = new FlxPoint(daNote.x, daNote.y);
				if(daNote.copyAngle && (daNote.mustPress || daNote.isSustainNote && daNote.mustPress)) {
					daNote.angle = strumAngle;
					urchild.rotate(urmom, playerStrums.members[daNote.noteData].angle);
					daNote.x = urchild.x;
					daNote.y = urchild.y;
				}
				*/
	...
	function openModChartEditor()
	{
		...
		cancelMusicFadeTween();
		CustomFadeTransition.nextCamera = camOther;
		MusicBeatState.switchState(new ModChartingState());
		chartingMode = true;
				#if desktop
	...
	public function checkEventNote() {
		while(eventNotes.length > 0) {
			...
			var value4:String = '';
			if(eventNotes[0][5] != null)
				value4 = eventNotes[0][5];

			var value5:String = '';
			if(eventNotes[0][6] != null)
				value5 = eventNotes[0][6];

			var value6:String = '';
			if(eventNotes[0][7] != null)
				value6 = eventNotes[0][7];

			triggerEventNote(eventNotes[0][1], value1, value2, value3, value4, value5, value6);
			eventNotes.shift();
		}
	}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String, value3:String, value4:String, value5:String, value6:String) {
		switch(eventName) {
			case 'Hey!':
				var value:Int = 2;
			...
			case 'be straight':
				...
				FlxTween.tween(playerStrums.members[0], {"angle": (playerStrums.members[0].angle - 45)}, 0.5, {ease: FlxEase.quadInOut});
				FlxTween.tween(playerStrums.members[1], {"angle": (playerStrums.members[1].angle - 22)}, 0.5, {ease: FlxEase.quadInOut});
				FlxTween.tween(playerStrums.members[2], {"angle": (playerStrums.members[2].angle + 22)}, 0.5, {ease: FlxEase.quadInOut});
				FlxTween.tween(playerStrums.members[3], {"angle": (playerStrums.members[3].angle + 45)}, 0.5, {ease: FlxEase.quadInOut});
			case 'be gay':
				FlxTween.tween(FlxG.camera, {zoom: (FlxG.camera.zoom + 0.1)}, 60 / SONG.bpm, {type: FlxTween.LOOPING, ease: FlxEase.quadOut});
			case 'be bi':
				FlxTween.tween(FlxG.camera, {zoom: (FlxG.camera.zoom + 0.1)}, 30 / SONG.bpm, {type: FlxTween.LOOPING, ease: FlxEase.quadOut});
			case 'be pan':
				FlxTween.tween(FlxG.camera, {zoom: (FlxG.camera.zoom + 0.2)}, 15 / SONG.bpm, {type: FlxTween.LOOPING, ease: FlxEase.quadOut});
			case 'move note inout':
				valya2 = Std.parseFloat(value2);
				valya3 = Std.parseInt(value3);
				valya5 = Std.parseFloat(value5);
				valya6 = Std.parseInt(value6);
				valya1 = Std.parseInt(value1);
				if (valya1 > 3)
				{
				...
				if (ween == true)
				{
					valya4 = -Std.parseInt(value4);
				}
				else
				{
					valya4 = Std.parseInt(value4);
				}
				FlxTween.tween(pissballs.members[valya1], {"y": (pissballs.members[valya1].ogposy + valya4)}, valya2, {ease: FlxEase.quadInOut});
				FlxTween.tween(pissballs.members[valya1], {"x": (pissballs.members[valya1].ogposx + valya3)}, valya2, {ease: FlxEase.quadInOut});
				FlxTween.tween(pissballs.members[valya1], {"angle": (valya6)}, valya2, {ease: FlxEase.quadInOut});
				FlxTween.tween(pissballs.members[valya1], {"alpha": (valya5)}, valya2, {ease: FlxEase.quadInOut});
			case 'move note in':
				valya2 = Std.parseFloat(value2);
				valya3 = Std.parseInt(value3);
				valya5 = Std.parseFloat(value5);
				...
				FlxTween.tween(pissballs.members[valya1], {"y": (pissballs.members[valya1].ogposy + valya4)}, valya2, {ease: FlxEase.quadIn});
				FlxTween.tween(pissballs.members[valya1], {"x": (pissballs.members[valya1].ogposx + valya3)}, valya2, {ease: FlxEase.quadIn});
				FlxTween.tween(pissballs.members[valya1], {"angle": (valya6)}, valya2, {ease: FlxEase.quadIn});
				FlxTween.tween(pissballs.members[valya1], {"alpha": (valya5)}, valya2, {ease: FlxEase.quadIn});
			case 'move note out':
				valya2 = Std.parseFloat(value2);
				valya3 = Std.parseInt(value3);
				valya5 = Std.parseFloat(value5);
				...
				FlxTween.tween(pissballs.members[valya1], {"y": (pissballs.members[valya1].ogposy + valya4)}, valya2, {ease: FlxEase.quadOut});
				FlxTween.tween(pissballs.members[valya1], {"x": (pissballs.members[valya1].ogposx + valya3)}, valya2, {ease: FlxEase.quadOut});
				FlxTween.tween(pissballs.members[valya1], {"angle": (valya6)}, valya2, {ease: FlxEase.quadOut});
				FlxTween.tween(pissballs.members[valya1], {"alpha": (valya5)}, valya2, {ease: FlxEase.quadOut});
			case 'move note linear':
				valya2 = Std.parseFloat(value2);
				valya3 = Std.parseInt(value3);
				valya5 = Std.parseFloat(value5);
				valya6 = Std.parseInt(value6);
				...
				FlxTween.tween(pissballs.members[valya1], {"y": (pissballs.members[valya1].ogposy + valya4)}, valya2);
				FlxTween.tween(pissballs.members[valya1], {"x": (pissballs.members[valya1].ogposx + valya3)}, valya2);
				FlxTween.tween(pissballs.members[valya1], {"angle": (valya6)}, valya2);
				FlxTween.tween(pissballs.members[valya1], {"alpha": (valya5)}, valya2);
	...
	private function onKeyRelease(event:KeyboardEvent):Void
	{
		...
		{
			var spr:StrumNote = playerStrums.members[key];
			var spr2:StrumNote = opponentStrums.members[key];
			trace(playerStrums.members[key].x - opponentStrums.members[key].x);
			if(spr != null)
			{
				spr.playAnim('static');
				spr.resetAnim = 0;
